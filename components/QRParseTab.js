...
const QRParseTab = ({ onParseComplete }) => {
  ...
  const parseQRCode = (qrCode) => {
    ...
    try {
      // QR kod formatı: 01BARKOD17YYMMDD21SERINO10PARTINO
      // 01: Application Identifier (Barkod)
      ...
      while (position < qrCode.length) {
        if (qrCode.substr(position, 2) === '01') {
          // Barkod (14 karakter)
          position += 2;
          result.barkod = qrCode.substr(position, 14);
          position += 14;
        }
        else if (qrCode.substr(position, 2) === '17') {
          // Son kullanma tarihi (6 karakter YYMMDD)
          position += 2;
          const dateStr = qrCode.substr(position, 6);
          if (dateStr.length === 6) {
        ...
        else if (qrCode.substr(position, 2) === '21') {
          // Seri numarası (değişken uzunluk, sonraki GS1 koduna kadar)
          position += 2;
          let seriLength = 0;
          for (let i = position; i < qrCode.length; i++) {
            if (qrCode.substr(i, 2) === '10' || qrCode.substr(i, 2) === '17' || qrCode.substr(i, 2) === '01') {
              break;
            }
            seriLength++;
          }
          result.seriNo = qrCode.substr(position, seriLength);
          position += seriLength;
        }
        else if (qrCode.substr(position, 2) === '10') {
          // Parti numarası (geri kalan kısım)
          position += 2;
          result.partiNo = qrCode.substr(position);
          break;
        }
        else {
          position++;
        ...
        <div className="bg-white/5 rounded-xl p-4 border border-white/10">
          <div className="flex items-start space-x-3">
            ...
            <div className="space-y-2">
              <h4 className="text-white font-medium">QR Parçalama Hakkında</h4>
              <ul className="text-white/70 text-sm space-y-1">
                <li>• Excel dosyalarında ilk sütundaki QR kodları işlenir</li>
                <li>• GS1 standardına uygun QR kodları desteklenir (01, 17, 21, 10)</li>
                <li>• Barkod (01), Son Kullanma Tarihi (17), Seri No (21), Parti No (10) bilgileri çıkarılır</li>
